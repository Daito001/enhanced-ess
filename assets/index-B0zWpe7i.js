...
Fortfahren?`)){s.style.pointerEvents="",s.style.opacity="",n.innerHTML=`
  ...
  `}async function Bs({params:e,root:t}){const a=e.yyyymm,[n,s,i,l]=await Promise.all([N(),ge(),U(),pe()]),c=s.find(g=>g.monatKey===a),o=Re(n,s).filter(g=>g.monatKey===a);if(!o.length){t.innerHTML=`<h1 class="view-title">Monat ${at(a)}</h1><p class="view-subtitle">Keine Daten für diesen Monat.</p><a href="#/monat" class="btn">← Alle Monate</a>`;return}const d=l!=null&&l.stichtagAuswertung?new Date(l.stichtagAuswertung):new Date,u=Pe(n,i,d).filter(g=>g.datum.startsWith(a));t.innerHTML=`
  ...
  `}function Is(e){var n;const t={};for(const s of e)t[n=s.wocheKey]??(t[n]={key:s.wocheKey,tage:[],ist:0,soll:0}),t[s.wocheKey].tage.push(s),t[s.wocheKey].ist+=ve(s),t[s.wocheKey].soll+=Ee(s);const a=Object.keys(t).sort();return a.length?a.map(s=>{var u;const i=t[s],l=+(i.ist-i.soll).toFixed(2),c=l>.05?"var(--success)":l<-.05?"var(--danger)":"var(--fg-muted)",r=(u=i.tage[0])==null?void 0:u.datum,o=r?Te(F(r)):"",d=s.split("-")[1]||"?";return`
  ...
  `}async function js({params:e,root:t}){const a=e.yyyykw,[n,s,i,l]=await Promise.all([N(),ge(),U(),pe()]),r=Re(n,s).filter(m=>m.wocheKey===a);if(!r.length){t.innerHTML=`<h1 class="view-title">KW ${w(a)}</h1><p>Keine Daten.</p><a href="#/woche" class="btn">← Alle Wochen</a>`;return}const o=r.reduce((m,b)=>m+ve(b),0),d=r.reduce((m,b)=>m+Ee(b),0),u=+(o-d).toFixed(2),g=m=>m>.05?"var(--success)":m<-.05?"var(--danger)":"var(--fg)",h=l!=null&&l.stichtagAuswertung?new Date(l.stichtagAuswertung):new Date,f=Pe(n,i,h).filter(m=>r.some(b=>b.datum===m.datum));t.innerHTML=`
    <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 8px;">
    ...
    <div class="grid cols-2" style="gap: 24px;">
      <div class="card">
        ...
        ${(d=n.zeitSpannen)!=null&&d.length?n.zeitSpannen.map(g=>`
              <div class="tag-detail">
                <div class="tag-detail-label">${Se(g.aktivitaet??"Stempelzeit")}</div>
      ...
      <div class="card">
        ...
        <div class="tag-detail">
          ...
          <div class="tag-detail-value">${n.tagestyp?`<span class="pill ${qs(n.tagestyp)}">${Se(n.tagestyp)}</span>`:"—"}</div>
        </div>
        <div class="tag-detail">
        ...
        <div class="tag-detail">
          ...
          <div class="tag-detail-value"><code>${Se(n.tazp??"—")}</code></div>
        </div>
        <div class="tag-detail">
          <div class="tag-detail-label">Monat</div>
          <div class="tag-detail-value">${Se(n.monatKey)}</div>
        </div>
      </div>
        ...
        <div class="card" style="grid-column: 1 / -1;">
          ...
          ${l.warnung?`
            <div class="alert severity-warn" style="margin-top: 12px;">
              ...
              <div class="alert-body">${Se(l.warnung)}</div>
            </div>
          `:""}
        </div>
      `:""}
    </div>
  `}function qs(e){const t=String(e).toLowerCase();return t.includes("stempel")?"stempelzeit":t.includes("berufs")?"berufsschule":t.includes("urlaub")?"urlaub":t.includes("feiertag")?"feiertag":t.includes("dienst")?"dienstreise":t.includes("gleit")?"gleitzeit":t.includes("arbeitsunf")?"arbeitsunfaehigkeit":"feiertag"}function Se(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}const Zs=[{key:14,label:"2 Wochen"},{key:30,label:"1 Monat"},{key:90,label:"3 Monate"},{key:"alles",label:"Alles"}];async function Gs({root:e}){var i;const[t,a]=await Promise.all([N(),ge()]);if(!t.length){e.innerHTML='<h1 class="view-title">Analytics</h1><p class="view-subtitle">Keine Daten.</p><a href="#/daten" class="btn primary">Zum Upload</a>';return}let n=t.length>30?30:"alles";e.innerHTML=`
...
Vielen Dank!`,s=new URLSearchParams;return s.set("cc","anika.kailer@putzmeister.com"),s.set("subject",a),s.set("body",n),`mailto:annika.mayer@putzmeister.com?${s.toString().replace(/\+/g,"%20")}`}function ti(e){const t=new Set;for(const a of e)a.tagestyp==="Berufsschule"&&(a.ist??0)>0&&t.add(a.monatKey);return[...t].sort().reverse()}function ai(e,t,a,n){var g,h;const s=t==null?void 0:t.berufsschulMuster;if(!s||!((g=s.wochentageWoechentlich)!=null&&g.length)&&!((h=s.wochentage14Taegig)!=null&&h.length))return{ausstehend:[],imMonatGesamt:a.length,vorhandenAnzahl:a.length};const[i,l]=e.split("-").map(f=>parseInt(f,10)),c=new Date(i,l-1,1),r=new Date(i,l,0),o=new Set(a.map(f=>f.datum)),d=[];let u=new Date(c);for(;u<=r;){if(!rt(u)&&je(u,s)){const f=E(u,"yyyy-MM-dd");u>n&&!o.has(f)&&d.push(f)}u=it(u,1)}return{ausstehend:d,imMonatGesamt:a.length+d.length,vorhandenAnzahl:a.length}}function ni(e){const t=e instanceof ArrayBuffer?e:e!=null&&e.buffer?e.buffer.slice(e.byteOffset,e.byteOffset+e.byteLength):null;if(!t||t.byteLength<24)return null;const a=new DataView(t);if(a.getUint32(0)===2303741511&&a.getUint32(4)===218765834)return{width:a.getUint32(16),height:a.getUint32(20)};if(a.getUint16(0)===65496){let n=2;for(;n+9<a.byteLength;){if(a.getUint8(n)!==255)return null;const s=a.getUint8(n+1);if(s>=192&&s<=207&&s!==196&&s!==200&&s!==204)return{height:a.getUint16(n+5),width:a.getUint16(n+7)};const l=a.getUint16(n+2);if(l<2)return null;n+=2+l}}return null}function si(e,t){const a=URL.createObjectURL(e),n=document.createElement("a");n.href=a,n.download=t,document.body.appendChild(n),n.click(),document.body.removeChild(n),setTimeout(()=>URL.revokeObjectURL(a),1e3)}async function Ae({root:e}){var b,p,z,I,k,T,L,R,D;const t=await ze();if(!t){ii(e);return}const[a,n,s]=await Promise.all([N(),U(),pe()]),i=s!=null&&s.stichtagAuswertung?new Date(s.stichtagAuswertung):new Date,l=ti(a),c=new Date,r=`${c.getFullYear()}-${String(c.getMonth()+1).padStart(2,"0")}`;l.includes(r)||l.unshift(r);const o=l;e.innerHTML=`
    ...
    `:(()=>{var _;const S=o.map((x,W)=>{const V=Pt(a,x),X=+(V.length*(t.konstanten.betragProTag??0)).toFixed(2),q=V.length>10,y=ai(x,n,V,i),B=y.ausstehend.length,Y=+((V.length+B)*(t.konstanten.betragProTag??0)).toFixed(2),be=B>0?y.ausstehend.map(he=>C(F(he))).join(", "):"";return{mk:x,bs:V,summe:X,ueberzaehlig:q,offen:B,offenListe:be,summeMitOffen:Y,idx:W}}),$=((_=S.find(x=>x.bs.length>0))==null?void 0:_.mk)??S[0].mk,M=S.find(x=>x.mk===$)??S[0];return`
  ...
  `:'<p style="color: var(--fg-muted);">Keine Tage in dieser Auswahl.</p>'}function di(e){return{MO:"Montag",DI:"Dienstag",MI:"Mittwoch",DO:"Donnerstag",FR:"Freitag",SA:"Samstag",SO:"Sonntag"}[e]??e}function ui(e){const[t,a]=e.split("-");return E(new Date(parseInt(t),parseInt(a)-1,1),"LLLL yyyy",{locale:J})}function gi(e){const t=String(e).toLowerCase();return t.includes("stempel")?"stempelzeit":t.includes("berufs")?"berufsschule":t.includes("urlaub")?"urlaub":t.includes("feiertag")?"feiertag":t.includes("dienst")?"dienstreise":t.includes("gleit")?"gleitzeit":t.includes("arbeitsunf")?"arbeitsunfaehigkeit":"feiertag"}function ca(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function Lt(e){return ca(e)}async function hi({root:e}){const[t,a,n,s]=await Promise.all([N(),U(),ge(),ct()]),i=Object.fromEntries(s.map(r=>[r.key,r])),l=Pe(t,a,new Date).filter(r=>r.mailVorlageKey),c=Zt(n);e.innerHTML=`
    ...
    <div class="card">
      ...
      <div id="vorlagen-editor">
        ${Object.keys(De).map(r=>{const o=Ne(r,i[r]);return mi(o)}).join("")}
      </div>
    </div>
...
${n}`;await navigator.clipboard.writeText(s),K("Mail-Text kopiert.")})})}function Z(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function st(e){return Z(e)}async function bi({root:e}){const[t,a]=await Promise.all([U(),ct()]),n=Object.fromEntries(a.map(s=>[s.key,s]));e.innerHTML=`
    ...
    <div class="grid cols-2">
      ...
      <div class="card" style="grid-column: 1 / -1;">
        ...
        <div id="vorlagen-editor">
          ${Object.keys(De).map(s=>{const i=Ne(s,n[s]);return yi(i)}).join("")}
        </div>
      </div>
