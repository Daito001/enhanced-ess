...
Wenn du fortfährst, werden alle alten Tagesdaten gelöscht und durch die neuen ersetzt. (Profil-Einstellungen wie Theme und Mail-Vorlagen bleiben erhalten.)`,{title:"Anderer Azubi erkannt",confirmLabel:"Daten ersetzen",variant:"danger"})){s.style.pointerEvents="",s.style.opacity="",a.innerHTML=`
  ...
  `}function sr(e){const t=String(e).toLowerCase();return t.includes("stempel")?"stempelzeit":t.includes("berufs")?"berufsschule":t.includes("urlaub")?"urlaub":t.includes("feiertag")?"feiertag":t.includes("dienst")?"dienstreise":t.includes("gleit")?"gleitzeit":t.includes("arbeitsunf")?"arbeitsunfaehigkeit":"feiertag"}function He(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}const ir=[{key:14,label:"2 Wochen"},{key:30,label:"1 Monat"},{key:90,label:"3 Monate"},{key:180,label:"6 Monate"},{key:365,label:"1 Jahr"},{key:730,label:"2 Jahre"},{key:"alles",label:"Alles"}];function rr(e){return ir.filter(t=>t.key==="alles"||t.key<e)}async function lr({root:e}){var r,o;const[t,n]=await Promise.all([ue(),ht()]);if(!t.length){e.innerHTML='<h1 class="view-title">Analytics</h1><p class="view-subtitle">Keine Daten.</p><a href="#/daten" class="btn primary">Zum Upload</a>';return}const a=rr(t.length);let s=a.some(u=>u.key===30)?30:"alles";e.innerHTML=`
  ...
  `;let i=0;const l=()=>{if(s==="alles")return t;const u=t.length,h=Math.min(s,u),g=u+i*h,p=g-h;return g<=0?[]:t.slice(Math.max(0,p),g)},d=()=>{const u=e.querySelector("#startend-nav");if(!u)return;const h=s==="alles";if(u.hidden=h,h)return;const g=lt(t.length,s),p=l();u.querySelector('.zeitraum-nav-btn[data-dir="prev"]').disabled=i<=g,u.querySelector('.zeitraum-nav-btn[data-dir="next"]').disabled=i>=0;const f=u.querySelector(".zeitraum-nav-label");f&&(f.textContent=Yn(p,s))},c=()=>{nn(e.querySelector("#c-startend"),l()),d()};requestAnimationFrame(()=>{Fs(e.querySelector("#c-saldo"),n),Os(e.querySelector("#c-wochen"),t),nn(e.querySelector("#c-startend"),l()),Cs(e.querySelector("#c-wochentag"),t),_s(e.querySelector("#c-pausen"),t),Bs(e.querySelector("#c-donut"),t),Ls(e.querySelector("#c-alle-tage"),t.slice(-30)),d()}),(r=e.querySelector("#startend-zeitraum"))==null||r.addEventListener("click",u=>{const h=u.target.closest("button[data-key]");if(!h||h.disabled)return;const g=h.getAttribute("data-key"),p=g==="alles"?"alles":parseInt(g,10);p!==s&&(s=p,i=0,e.querySelectorAll("#startend-zeitraum button").forEach(f=>f.classList.toggle("active",f===h)),c())}),(o=e.querySelector("#startend-nav"))==null||o.addEventListener("click",u=>{const h=u.target.closest(".zeitraum-nav-btn");if(!h||h.disabled||s==="alles")return;const g=h.getAttribute("data-dir")==="prev"?-1:1,p=lt(t.length,s);let f=i+g;f<p&&(f=p),f>0&&(f=0),f!==i&&(i=f,c())})}function or(e){return String(e??"").replace(/"/g,"&quot;")}async function St({params:e,root:t}){const[n,a,s,i,l,d]=await Promise.all([ue(),ht(),oe(),Xe(),ft(),Le()]),c=Object.fromEntries(l.map(b=>[b.key,b]));if(!n.length){t.innerHTML=`
