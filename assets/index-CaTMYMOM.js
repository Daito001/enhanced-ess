import{D as tt}from"./dexie-BODkcwel.js";import{b as ne,a as H,c as Z,d as at,f as x,e as U,i as We,p as nt,g as st,h as it,j as z,k as Ke,l as rt,s as lt,m as ot}from"./datefns-Dq49jaKA.js";import{C as T,L as ct,B as dt,D as ut,S as gt,a as ht,b as mt,P as ft,A as pt,c as vt,d as bt,T as yt,p as wt,e as $t,i as Mt}from"./chartjs-CRgZvhTJ.js";import{_ as zt,a as xt}from"./pdfjs-BDGsuRSB.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const l of i.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&n(l)}).observe(document,{childList:!0,subtree:!0});function a(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(s){if(s.ep)return;s.ep=!0;const i=a(s);fetch(s.href,i)}})();const ae="theme";function St(){const e=localStorage.getItem(ae)??"auto";oe(e)}function oe(e){e==="auto"?document.documentElement.removeAttribute("data-theme"):document.documentElement.setAttribute("data-theme",e),localStorage.setItem(ae,e),At(e)}function kt(){const e=localStorage.getItem(ae)??"auto",t=e==="auto"?"light":e==="light"?"dark":"auto";return oe(t),Oe(t),t}const se=new Set;function Dt(e){return se.add(e),()=>se.delete(e)}function Oe(e){for(const t of se)try{t(e)}catch(a){console.error(a)}}var Fe;if(typeof window<"u"&&window.matchMedia){const e=window.matchMedia("(prefers-color-scheme: dark)");(Fe=e.addEventListener)==null||Fe.call(e,"change",()=>{Ce()==="auto"&&Oe("auto")})}function At(e){const t=document.getElementById("theme-toggle");if(!t)return;const a=t.querySelector(".theme-label"),n=t.querySelector(".theme-icon-dark"),s=t.querySelector(".theme-icon-light");a&&(e==="auto"?(a.textContent="Auto",n&&(n.style.display="inline"),s&&(s.style.display="inline")):e==="light"?(a.textContent="Hell",n&&(n.style.display="none"),s&&(s.style.display="inline")):(a.textContent="Dunkel",n&&(n.style.display="inline"),s&&(s.style.display="none")))}function Ce(){return localStorage.getItem(ae)??"auto"}const Ie=[];let Be=null;function k(e,t){const a=e.split("/").filter(Boolean),n=[],s=new RegExp("^/?"+a.map(i=>i.startsWith(":")?(n.push(i.slice(1)),"([^/]+)"):Ft(i)).join("/")+"/?$");Ie.push({pattern:e,regex:s,paramNames:n,render:t})}function Pt(e){e.startsWith("#")||(e="#"+e),window.location.hash===e?L():window.location.hash=e}function Tt(e,{onNavigate:t}={}){window.addEventListener("hashchange",L),L.rootEl=e,L.onNavigate=t,L()}async function L(){var n;const e=L.rootEl,t=window.location.hash.slice(1)||"/",a=t.startsWith("/")?t:`/${t}`;for(const s of Ie){const i=s.regex.exec(a);if(i){const l={};s.paramNames.forEach((r,o)=>l[r]=decodeURIComponent(i[o+1])),Be=a,(n=L.onNavigate)==null||n.call(L,a),Lt(a);try{e.classList.remove("view-enter"),await s.render({params:l,path:a,root:e}),e.classList.add("view-enter")}catch(r){console.error("Route render error:",r),e.innerHTML=`<div class="empty-state">
      ...
      <div style="flex: 1; min-width: 400px; display: flex; flex-direction: column;">
        <div class="card-split" style="margin-bottom: 12px; align-items: center;">
          ...
          <div class="segmented" id="zeitraum-picker">
            ${_a.map(w=>{const E=typeof w.key=="number"&&w.key>o;return`<button type="button" data-key="${w.key}" class="${c===w.key?"active":""}" ${E?'disabled style="opacity:0.4"':""}>${w.label}</button>`}).join("")}
          </div>
        </div>
  ...
  `,ze(c,e,r),e.querySelectorAll("#zeitraum-picker button").forEach(w=>{w.addEventListener("click",async()=>{if(w.disabled)return;const E=w.getAttribute("data-key"),j=E==="alles"?"alles":parseInt(E,10);e.querySelectorAll("#zeitraum-picker button").forEach(_=>_.classList.toggle("active",_===w)),await Ot("dashboardZeitraum",j),ze(j,e,r)})}),requestAnimationFrame(()=>{Je(e.querySelector("#mini-heatmap"),n,{monate:4})}),e.addEventListener("click",async w=>{const E=w.target.closest(".btn-dismiss");if(!E)return;const j=E.getAttribute("data-datum"),_=E.getAttribute("data-typ");if(!j||!_)return;await Zt(j,_),A("Warnung ausgeblendet.");const be=E.closest(".alert");be&&(be.style.display="none")})}function ze(e,t,a){const n=Ta(a,e),s=Ea(n);requestAnimationFrame(()=>{const o=t.querySelector("#saldo-chart");Fa(o,n)});const i=t.querySelector("#hero-bewegung");if(i){const o=s>.05?"up":s<-.05?"down":"flat",c=s>.05?"↑":s<-.05?"↓":"→";i.className=`saldo-trend ${o}`,i.innerHTML=`${c} ${Math.abs(s)<.01?"Unverändert":D(s,{signed:!0})} im Zeitraum`}const l=t.querySelector("#tage-info");if(l){const o=n.length;l.textContent=`${o} ${o===1?"Tag":"Tage"}`}const r=t.querySelector("#tage-liste");r&&(r.innerHTML=Ha(n.slice().reverse()))}function Ha(e){if(!e.length)return'<p class="hint">Keine Tage im Zeitraum.</p>';const t=Math.max(1,...e.map(a=>Math.abs(a.diff??0)));return`
      ...
      ${e.map(a=>{const n=a.diff??0,s=n>0,i=Math.min(100,Math.abs(n)/t*100),l=s?"var(--success)":n<0?"var(--danger)":"var(--fg-subtle)";return`
          <a href="#/tag/${a.datum}" class="tag-row">
            <div class="tag-row-datum">
              <div class="tabular">${F(b(new Date(a.datum)))}</div>
            ...
            <div class="tag-row-ist tabular">
              <div>${a.ist!=null?a.ist.toFixed(2).replace(".",",")+"h":"—"}</div>
